...
export default function FeaturedLesson({ job }: { job: Job }) {
    ...
    return (
        <div className="relative w-full bg-gray-900 rounded-[2rem] overflow-hidden shadow-2xl flex flex-col md:flex-row group mb-8">
            ...
            <div className="relative z-10 flex-1 p-10 md:p-14 flex flex-col justify-center items-start gap-6">
                <div className="inline-flex items-center gap-2 px-3 py-1 bg-green-500/20 border border-green-500/30 rounded-full text-green-400 text-xs font-black uppercase tracking-widest">
                ...
                <div className="flex items-center gap-4 mt-4">
                    <Link href={href} className={`px-8 py-4 rounded-xl font-black transition-colors flex items-center gap-2 active:scale-95 ${job.status === "DONE" ? "bg-green-500 hover:bg-green-400 text-gray-900 shadow-lg shadow-green-500/20" : "bg-gray-700 text-gray-400 cursor-not-allowed"}`}>
                        {job.status === "DONE" ? "Start Lesson" : "Processing..."}
                        {job.status === "DONE" && <Play className="w-5 h-5 fill-current" />}
                    </Link>
                    <button className="px-8 py-4 rounded-xl font-bold bg-white/10 text-white hover:bg-white/20 transition-colors">
                        View Details
                    </button>
                </div>
            </div>
